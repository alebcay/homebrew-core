class StripNondeterminism < Formula
  ...
  def install
    ...
    # Only keep the executable/man page for strip-nondeterminism
    rm_f Dir[libexec/"bin/*"].reject { |f| f == "#{libexec}/bin/#{name}" }
    rm_f Dir[libexec/"man/man1/*"].reject { |f| f == "#{libexec}/man/man1/#{name}.1" }

    # Don't keep manpages for internal dependencies/handlers
    rm_rf libexec/"man/man3"

    # Remove unnecessary metadata/receipts left behind by perl make process
    rm_f Dir[libexec/"**/{perllocal.pod,.packlist}"]

    # Remove nondeterministic aspects from manpages
    inreplace Dir[libexec/"man/**/*.[1-8]"] do |s|
      s.gsub!(/"perl\s+v.*"/, "\"perl\"")
      s.gsub!(/^\.\\" Automatically generated by.*$/, "")
    end

    (bin/name).write_env_script libexec/"bin"/name, PERL5LIB: ENV["PERL5LIB"]
    man1.install_symlink libexec/"man/man1/#{name}.1"
  end
